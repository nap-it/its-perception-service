...
//CAM variables
float cam_latitude = 0;
float cam_longitude =  0;
...
void adapter_handler(const string& response){
    ...
    // Check if it is a sensorType message
    ...
    while(1) {
        try {
            ...
            if (current_request - last_request >= request_interval) {
                ...
                server->publish("to/adapters", "{\"requestID\":" + to_string(timestamp_milliseconds) + ",\"numberObjects\":" + to_string(0) + "}");
                spdlog::debug("Sent request to {} with ID {} and numberObjects 0", "to/adapters", timestamp_milliseconds);
                ...
                for (const auto& cpm_str : cpmList) {
                    ...
                    spdlog::debug("[{}] CPM Published: {}", cpm_str);
                }
                ...
                auto ending_time = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();
                spdlog::info("Total time to publish CPM: {} with ID {}", (ending_time - current_request.count()), timestamp_milliseconds);
