...
struct Object {
    int objectID;                      ///< Raw object identifier from the camera pipeline.
...
public:
    /**
     * @brief Construct a new Camera Adapter object.
     ...
     * Subscribes to MQTT topics and processes incoming camera detections.
     */
    void run();
    void stop();
...
private:
    ...
    MqttWrapper* mqtt_wrapper;        ///< MQTT client for subscribing to camera topics.
     ...
     * Not used in this adapter (camera input is MQTT only).
